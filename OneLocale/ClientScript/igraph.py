...
def ak_connect():
    ...
    print("Graph Components Analysis")
    ...
    MtxEdgeList=[ [106762,31379,3,0,HomeDir+"Adata/SNAP/as-caida.mtx"],\
            ]
...
#            [936364282,39459925,2,0,HomeDir+"Adata/SNAP/uk-2005.mtx"],\
    ...
    for i in MtxEdgeList:
        ...
        reader = nk.graphio.EdgeListReader(separator=" ")
        graph=reader.read(str(FileName))
        start_time = time.time()
        cc = nk.components.ConnectedComponents(graph)
        cc.run()
        end_time=time.time()
        print(f"number of components ={cc}, execution time={end_time-start_time}")
    for i in BigMtxFile:
        Edges=i[0]
        Vertices=i[1]
        Columns=i[2]
        ...
        FileName=i[4]
        print(Edges,",",Vertices,",",Columns,",",Directed,",",str(FileName))
        Graph=ig.Graph.Read_MatrixMarket(str(FileName))
        start_time = time.time()
