...
iteration_numbers = data.iloc[:, [1,7,8,9]]
...
plt.savefig(filename+"-IterNumUP-.png")
plt.show()
plt.close()

execution_values = data.iloc[:, [10,16,17,18]]
print(execution_values)
exetime=execution_values.to_numpy()
pda=pd.DataFrame(exetime,columns = range(1,5))
...
#plt.subplots_adjust(bottom=0.19)
plt.savefig(filename+"-ExeTimeUP-.png")
plt.show()
plt.close()
...
ax.legend(["FastSV","UP-","UP","UPS"],ncol=4)
#ax.set_yscale('symlog', basey=2)
#plt.yscale('log',base=2) 
plt.ylabel("Speedup")
plt.xlabel("Graph ID")
...
all.to_csv(filename+"-UP-all-.csv")
#all=pd.concat([data, speedup],axis=1)
